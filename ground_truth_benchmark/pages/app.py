...
# View and Upload Documents Page
    ...
    # Ensure SharePoint session exists
    if "ctx" not in st.session_state:
        st.warning("🔑 Please log in to SharePoint first.")
    else:
        ctx = st.session_state["ctx"]
        folder_in_sharepoint = "/sites/Test/Document/Test PDFs"

        # Navigation menu
        Pages = option_menu("", ["📂 File List", "📤 Upload New File"], default_index=0, orientation="horizontal")

        # Fetch SharePoint folder
        ...
        # File List Page
        if Pages == "📂 File List":
            file_data = [{"📄 File Name": item.properties["Name"], "📅 Last Modified": item.properties["TimeLastModified"]}
                         for item in items]
            
            if file_data:
                df = pd.DataFrame(file_data)
                st.write(" **Files in SharePoint Folder:**")
                st.table(df)
            else:
                st.warning("⚠️ No files found in the SharePoint folder.")

        # Upload New File Page
        elif Pages == "📤 Upload New File":
            ...
            if uploaded_file:
                ...
                st.write(f"✅ **File Ready to Upload:** `{file_name}` ({len(file_bytes)} bytes)")

                if st.button("📤 Upload File to SharePoint"):
                    try:
